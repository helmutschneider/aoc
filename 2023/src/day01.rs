pub fn part1() {
    ...
    for line in INPUT.trim().lines() {
        let num = find_two_digit_number(line);
        sum += num;
    }
...
pub fn part2() {
    let mut sum: i64 = 0;
    for line in INPUT.trim().lines() {
        let num = find_two_digit_number_giga(line);
        sum += num;
    }
...
const ZERO_AS_ASCII: u8 = b'0';
const NUMBERS_AS_WORDS: [(&'static str, i64); 9] = [
    ("one", 1),
    ("two", 2),
    ("three", 3),
    ("four", 4),
...
fn find_two_digit_number(thing: &str) -> i64 {
    let bytes = thing.as_bytes();
    ...
    return (first.unwrap() - ZERO_AS_ASCII) as i64 * 10 + (last.unwrap() - ZERO_AS_ASCII) as i64;
}

fn find_two_digit_number_giga(thing: &str) -> i64 {
    ...
    for i in 0..len {
        let bt = bytes[i];
        if bt.is_ascii_digit() {
            first = Some((bt - ZERO_AS_ASCII) as i64 * 10);
        }
        for (maybe_s, value) in NUMBERS_AS_WORDS {
            let num_len = maybe_s.len();
            let end_index = core::cmp::min(i + num_len, len);

            if &thing[i..end_index] == maybe_s {
                first = Some(value * 10);
                break;
            }
        }
    ...
    for i in 0..len {
        ...
        let bt = bytes[index];
        if bt.is_ascii_digit() {
            last = Some((bt - ZERO_AS_ASCII) as i64);
        }
        for (maybe_s, value) in NUMBERS_AS_WORDS {
            let num_len = maybe_s.len();
            let start_index = core::cmp::max(0, index as i64 - num_len as i64 + 1) as usize;
            let end_index = index + 1;
            let maybe_other_s = &thing[start_index..end_index];

            if maybe_other_s == maybe_s {
                last = Some(value);
                break;
            }
        }
...
mod tests {
    ...
    #[test]
    fn find_num_1() {
        let x = find_two_digit_number("1abc2");
    ...
    #[test]
    fn find_num_2() {
        let x = find_two_digit_number("pqr3stu8vwx");
        assert_eq!(38, x);
    }

    #[test]
    fn find_num_3() {
        let x = find_two_digit_number_giga("two1nine");
        assert_eq!(29, x);
    }

    #[test]
    fn find_num_4() {
        let x = find_two_digit_number_giga("eightwothree");
        assert_eq!(83, x);
    }
}
