...
public class Day07
{
    ...
    static Card ParseCard(char ch, Part part)
    {
        return ch switch
        {
            '1' => Card.One,
            ...
            '9' => Card.Nine,
            'T' => Card.Ten,
            'J' when part == Part.One => Card.Jack,
            'J' when part == Part.Two => Card.Joker,
            'Q' => Card.Queen,
            'K' => Card.King,
            'A' => Card.Ace,
            _ => throw new InvalidOperationException($"cannot parse card '{ch}'"),
        };
    }

    abstract record HandType(int Ordering)
    {
        public record FiveOfAKind(Card Card) : HandType(6);
        public record FourOfAKind(Card Card) : HandType(5);
        public record FullHouse(Card TwoOf, Card ThreeOf) : HandType(4);
        public record ThreeOfAKind(Card Card) : HandType(3);
        public record TwoPair(Card PairA, Card PairB) : HandType(2);
        ...
        public static HandType FromHand(Hand hand, Part part)
        {
            ...
            {
                return new TwoPair(firstPairCard, secondPairCard);
            }

            if (firstPairCard != Card.None)
            {
    ...
    record Hand(IReadOnlyList<Card> Cards, long Bid, Part Part) : IComparable<Hand>
    {
        ...
        public int CompareTo(Hand? maybeOther)
        {
            if (maybeOther is Hand other)
            {
                ...
                return typeA.Ordering - typeB.Ordering;
            }
            return 1;
        }
    }

    static IReadOnlyList<Hand> Parse(string input, Part part)
    {
        return input.Trim()
            .Split("\n")
            .Select(line => Hand.Parse(line.Trim(), part))
            .ToList();
    }
